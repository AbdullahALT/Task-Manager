public class BST<T> {
	...
	private BSTNode<T> remove_aux(int key, BSTNode<T> p) {
	...
	private boolean recFind(BSTNode<T> node, int key) {
		if (node == null)
			return false;
		current = node;
		if (node.key == key)
			return true;
		if (node.key > key)
			return recFind(node.right, key);
		return recFind(node.left, key);
	}
